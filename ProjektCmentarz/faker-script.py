...
# Wypełnienie tabeli słownikowej GravestoneInscriptions
inscryption_strings = [
    'Offline na zawsze',
...
section_names = ['Sektor A', 'Sektor B', 'Sektor C', 'Sektor D']
cursor.execute("SELECT SectionType FROM GraveyardSection")  # <--- Zmienione na SectionType
existing_sections = {row[0] for row in cursor.fetchall()}

for name in section_names:
    if name not in existing_sections:
        cursor.execute("INSERT INTO GraveyardSection (SectionType) VALUES (?)", name)  # <--- Zmienione na SectionType

conn.commit()
print("Wypełniono tabelę GraveyardSection !")
